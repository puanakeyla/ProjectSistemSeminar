              ...
              <div className="status-card-body">
                      ...
                      {item.approvals.map((approval, idx) => (
                        <div key={idx} className="approval-item">
                          <span className="approval-role">
                            {approval.peran === 'pembimbing1' ? '👨‍🏫 Pembimbing 1' : 
                             approval.peran === 'pembimbing2' ? '👨‍🏫 Pembimbing 2' : 
                             '👨‍⚖️ Penguji'}
                          </span>
                ...
                {item.admin_status && (
                  <div className="info-row">
                    ...
                    <span className={`info-value admin-status ${item.admin_status}`}>
                      {item.admin_status === 'approved' ? '✅ Disetujui' :
                       item.admin_status === 'rejected' ? '❌ Ditolak' :
                       '⏳ Menunggu'}
                    </span>
                  </div>
                )}
                ...
                {item.schedule && (
                  <div className="schedule-info">
                    <span className="info-label">📅 Jadwal Seminar:</span>
